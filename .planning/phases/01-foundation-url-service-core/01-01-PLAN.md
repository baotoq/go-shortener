...
must_haves:
  ...
  artifacts:
    ...
    - path: "db/query.sql"
      ...
      contains: "CreateURL"
    - path: "internal/database/migrations/000001_create_urls.up.sql"
  ...
  key_links:
    ...
    - from: "db/schema.sql"
      to: "internal/database/migrations/000001_create_urls.up.sql"
...
Note: sqlc is a CLI tool installed separately (not a Go dependency). Install via:
  ...
  <verify>
    ...
    - Directory structure exists: cmd/url-service/, internal/domain/, internal/usecase/, internal/repository/sqlite/sqlc/, internal/delivery/http/, internal/database/migrations/, db/, pkg/problemdetails/
...
<task type="auto">
  ...
  <action>
1. Create `internal/database/migrations/000001_create_urls.up.sql`:
   ```sql
   CREATE TABLE urls (
...
2. Create `internal/database/migrations/000001_create_urls.down.sql`:
   ```sql
...
7. Verify generated files exist: db.go, models.go, query.sql.go, querier.go (or similar).
  ...
  <verify>
    ...
    - Migration files exist in `internal/database/migrations/` and contain correct SQL
