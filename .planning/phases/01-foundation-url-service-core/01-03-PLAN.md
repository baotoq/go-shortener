...
3. Update `cmd/url-service/main.go` — the full application entry point:
   ...
   - Run migrations using golang-migrate with embedded FS:
     - Embed migrations: `//go:embed migrations/*.sql` on a `var migrationsFS embed.FS` in main package
     - NOTE: Migration files must be in `cmd/url-service/migrations/` OR use a separate package. Since migrations/ is at project root, either:
       (a) Move migrations to cmd/url-service/migrations/ and embed from there, OR
       (b) Create a separate internal/database package that embeds and exports migrations
     - Recommended approach (b): Create `internal/database/database.go` that:
       - Embeds `../../migrations/*.sql` — WAIT, embed paths must be relative to the file. Better approach:
       - Place the embed directive in a `migrations/embed.go` file that lives next to the .sql files
       - OR simply copy/move migration files to where they can be embedded
     - SIMPLEST approach: Create `internal/database/migrate.go` and put migration SQL files as `internal/database/migrations/*.sql`. Copy the SQL content from the root migrations/ folder. The root migrations/ folder stays as reference but the embedded ones live with the Go code.
     - ALTERNATIVELY: Keep migrations at root, and in main.go use `os.DirFS` with file-based source instead of embed. But embed is better for single-binary deployment.
     - BEST approach for clean architecture: Keep migration .sql files at project root `migrations/` directory. In `cmd/url-service/main.go`, since go:embed requires the pattern to be relative to the source file, create a small helper:
       ```
       cmd/url-service/
         main.go          (imports internal/database)
       internal/database/
         database.go       (OpenDB, RunMigrations)
         migrations/       (embedded .sql files here)
           000001_create_urls.up.sql
           000001_create_urls.down.sql
       ```
     - Use this approach. Move migration files to `internal/database/migrations/` and embed from there. Remove root `migrations/` or keep as symlink/reference.
   - Wire dependencies:
     ```go
   ...
   **Files also modified:**
   - `internal/database/database.go` — new file for DB initialization + migration
   - `internal/database/migrations/000001_create_urls.up.sql` — moved from root
   - `internal/database/migrations/000001_create_urls.down.sql` — moved from root
  </action>
  <verify>
