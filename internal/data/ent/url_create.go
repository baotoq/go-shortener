...
func (uc *URLCreate) SetShortenedURL(s string) *URLCreate {
	uc.mutation.SetShortenedURL(s)
	return uc
}
...
func (uc *URLCreate) check() error {
	...
	if _, ok := uc.mutation.ShortenedURL(); !ok {
		return &ValidationError{Name: "shortened_url", err: errors.New(`ent: missing required field "Url.shortened_url"`)}
	}
	return nil
}
...
func (uc *URLCreate) createSpec() (*Url, *sqlgraph.CreateSpec) {
	...
	if value, ok := uc.mutation.ShortenedURL(); ok {
		_spec.SetField(url.FieldShortenedURL, field.TypeString, value)
		_node.ShortenedURL = value
	}
	return _node, _spec
}
