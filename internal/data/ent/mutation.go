...
// SetShortenedURL sets the "shortened_url" field.
func (m *URLMutation) SetShortenedURL(s string) {
	m.shortened_url = &s
}

// ShortenedURL returns the value of the "shortened_url" field in the mutation.
func (m *URLMutation) ShortenedURL() (r string, exists bool) {
	v := m.shortened_url
	if v == nil {
		return
	}
...
// OldShortenedURL returns the old "shortened_url" field's value of the Url entity.
// If the Url object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *URLMutation) OldShortenedURL(ctx context.Context) (v string, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldShortenedURL is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
		return v, errors.New("OldShortenedURL requires an ID field in the mutation")
	}
	oldValue, err := m.oldValue(ctx)
	if err != nil {
		return v, fmt.Errorf("querying old value for OldShortenedURL: %w", err)
	}
	return oldValue.ShortenedURL, nil
}

// ResetShortenedURL resets all changes to the "shortened_url" field.
func (m *URLMutation) ResetShortenedURL() {
	m.shortened_url = nil
}
...
func (m *URLMutation) Fields() []string {
	...
	if m.shortened_url != nil {
		fields = append(fields, url.FieldShortenedURL)
	}
	return fields
}
