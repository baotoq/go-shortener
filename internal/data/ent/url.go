...
type Url struct {
	...
	// ShortenedURL holds the value of the "shortened_url" field.
...
func (*Url) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case url.FieldOriginalURL, url.FieldShortenedURL:
...
func (u *Url) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case url.FieldShortenedURL:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field shortened_url", values[i])
			} else if value.Valid {
				u.ShortenedURL = value.String
			}
		default:
...
func (u *Url) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("shortened_url=")
	builder.WriteString(u.ShortenedURL)
	builder.WriteByte(')')
